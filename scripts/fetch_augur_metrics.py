...
for org in PROJECTS_TRACKED['projects']:
    for repo in PROJECTS_TRACKED['projects'][org]:
        print(f"Sending request to http://twitter.augurlabs.io/api/unstable/{org}/{repo}/bus_factor")
        r = requests.get(f"http://twitter.augurlabs.io/api/unstable/{org}/{repo}/bus_factor")
        try:
            if r.ok:
                print("OK!")
                bus_factor = r.json()[0]
            ...
            print("Error: Something went wrong with bus_factor - ", e)
